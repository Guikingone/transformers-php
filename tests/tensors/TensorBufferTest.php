...
it('can create a zero-sized buffer', static function (): void {
    $buffer = new TensorBuffer(0, Tensor::float32);

    expect($buffer->count())->toBe(0);
});

it('gets the correct value at the given offset using square brackets', function (): void {
    expect($this->tensorBuffer[0])->toBe(0.0)
        ->and($this->tensorBuffer[4])->toBe(0.0);
});

it('sets the value at the given offset using square brackets', function (): void {
...
it('throws an exception when unsetting offset using square brackets', function (): void {
